...
resource "yandex_dns_recordset" "rs_w_node" {                                             # Добавляем запись для master-node 
  name = "w-node"
  zone_id = yandex_dns_zone.private_zone.id
  type = "A"
  ttl = 200
  data = [yandex_compute_instance.w_node.network_interface.0.ip_address]
}
...
resource "yandex_compute_instance" "m_node" {                                             # Создаем master-node для подключения к внутренней инфре извне
...
resource "yandex_compute_instance" "w_node" {                                             # Создаем worker-node для подключения к внутренней инфре извне
  name = "w-node"                                                                         # необходимо предварительно сгенерировать пару rsa ключей в домашней дериктории
  zone = "ru-central1-a"                                                                  # образ машины - debian 12
  resources {                                                                                                                        
    cores = 2                                                                   
    memory = 2                                                                  
